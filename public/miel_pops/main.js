...
function init_threeScene(spec) {
  ...
  loaderFrame.load(
    './miel_pops/models/glasses/frame.json',
    (geometry) => {
  ...
  loaderLenses.load(
    './miel_pops/models/glasses/lenses.json',
    (geometry) => {
      const mat = new THREE.MeshBasicMaterial({
        map: new THREE.TextureLoader().load('./miel_pops/models/glasses/texture_mp.jpg')
      });
  ...
  loaderBranches.load(
    './miel_pops/models/glasses/branches.json',
    (geometry) => {
      const mat = new THREE.MeshBasicMaterial({
        alphaMap: new THREE.TextureLoader().load('./miel_pops/models/glasses/alpha_branches.jpg'),
        map: new THREE.TextureLoader().load('./miel_pops/models/glasses/textureBlack.jpg'),
        transparent: true
      });
  ...
  loaderDeco.load(
    './miel_pops/models/glasses/deco.json',
    (geometry) => {
      const mat = new THREE.MeshBasicMaterial({
        color: 0xffffff
      });
  ...
  beeLoader.load(
    './miel_pops/models/bee/bee.json',
    (geometry) => {

      const materialBee = new THREE.MeshLambertMaterial({
        map: new THREE.TextureLoader().load('./miel_pops/models/bee/texture_bee.jpg'),
        transparent: true,
        morphTargets: true
      });
  ...
  const calqueMesh = new THREE.Mesh(threeStuffs.videoMesh.geometry,  create_mat2d(new THREE.TextureLoader().load('./miel_pops/images/frame.png'), true))
